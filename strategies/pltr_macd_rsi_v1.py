"""PLTR MACD + RSI Momentum Strategy v1 (10m).

MACD histogram momentum with RSI confirmation and EMA trend filter.
Adapted for 10m bars. Tight stops, 2.3x+ targets. Long AND short.
"""
...
class Strategy(BaseStrategy):
    name = "PLTR MACD RSI Momentum"
    version = "v1"
    description = "MACD hist + RSI + EMA trend, 1x ATR stop / 2.5x target (10m)"
    ticker = "PLTR"
    timeframe = "10m"
    ...
    def on_bar(self, idx: int, row: pd.Series,
        ...
        if position is None:
            ...
            if macd_bull and trend_bull and rsi > 50 and vol_ok:
                return Signal(
                    direction="long",
                    stop_loss=close - stop_dist,
                    take_profit=close + target_dist,
                    reason=f"MACD bull, RSI {rsi:.0f}, EMA aligned"
                )

            # SHORT
            macd_bear = (hist < 0 and hist_falling) or hist_crossed_down
            trend_bear = close < fast_ema and fast_ema < slow_ema
            if macd_bear and trend_bear and rsi < 50 and vol_ok:
                return Signal(
                    direction="short",
                    stop_loss=close + stop_dist,
                    take_profit=close - target_dist,
                    reason=f"MACD bear, RSI {rsi:.0f}, EMA aligned"
                )

        if position is not None:
            if position.direction == "long" and (hist_crossed_down or close < slow_ema):
                return Signal(direction="close_long", reason="MACD flipped bearish")
            if position.direction == "short" and (hist_crossed_up or close > slow_ema):
                return Signal(direction="close_short", reason="MACD flipped bullish")
