"""PLTR Stochastic RSI + EMA Strategy v1 (10m).

Stochastic crossovers with RSI and EMA trend filter.
...
class Strategy(BaseStrategy):
    name = "PLTR Stoch RSI"
    version = "v1"
    description = "Stochastic + RSI + EMA trend, tight stops 2.3x R:R (10m)"
    ticker = "PLTR"
    timeframe = "10m"
    ...
    pine_conditions = {
        "long_entry": "STOCHk crosses above STOCHd and STOCHk < 30 and close > slow_ema",
        "short_entry": "STOCHk crosses below STOCHd and STOCHk > 70 and close < slow_ema",
    }
    ...
    def on_bar(self, idx: int, row: pd.Series,
        ...
        if position is None:
            if k_crossed_up and k < self.params["stoch_overbought"]:
                if close > slow and rsi > 40 and rsi < 75 and vol_ok:
                    return Signal(
                        direction="long",
            ...
            if k_crossed_down and k > self.params["stoch_oversold"]:
                if close < slow and rsi < 60 and rsi > 25 and vol_ok:
                    return Signal(
                        direction="short",
                        stop_loss=close + stop_dist,
                        take_profit=close - target_dist,
        ...
        if position is not None:
            if position.direction == "long" and k_crossed_down and k > 80:
                return Signal(direction="close_long", reason=f"Stoch overbought cross down")
            if position.direction == "short" and k_crossed_up and k < 20:
                return Signal(direction="close_short", reason=f"Stoch oversold cross up")
