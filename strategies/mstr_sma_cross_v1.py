"""MSTR SMA Crossover Strategy v1.

Uses SMA 10/30 crossover for 5-minute day trading.
SMA is smoother than EMA -- fewer false signals but slower entries.
Adds RSI filter to avoid buying overbought / selling oversold.
...
class Strategy(BaseStrategy):
    name = "MSTR SMA Cross"
    version = "v1"
    description = "SMA 10/30 crossover with RSI filter and volume confirmation"
    ticker = "MSTR"
    timeframe = "5m"
    ...
    def on_bar(self, idx: int, row: pd.Series,
        ...
        # LONG: SMA cross up + RSI not overbought + volume confirmation
        if position is None and cross_above and rsi < self.params["rsi_overbought"] and vol_ok:
            return Signal(
                ...
                reason=f"SMA bullish cross, RSI {rsi:.0f}, vol {volume:.0f} > avg {avg_volume:.0f}"
            )

        # SHORT: SMA cross down + RSI not oversold + volume confirmation
        if position is None and cross_below and rsi > self.params["rsi_oversold"] and vol_ok:
            return Signal(
                direction="short",
                stop_loss=close * (1 + self.params["stop_loss_pct"]),
                take_profit=close * (1 - self.params["take_profit_pct"]),
                reason=f"SMA bearish cross, RSI {rsi:.0f}, vol {volume:.0f} > avg {avg_volume:.0f}"
            )

        # EXIT LONG: bearish cross OR RSI extreme overbought
        if position is not None and position.direction == "long":
            if cross_below or rsi > self.params["rsi_exit_ob"]:
                return Signal(direction="close_long",
                              reason=f"SMA bearish cross or RSI {rsi:.0f} > {self.params['rsi_exit_ob']}")

        # EXIT SHORT: bullish cross OR RSI extreme oversold
        if position is not None and position.direction == "short":
            if cross_above or rsi < self.params["rsi_exit_os"]:
                return Signal(direction="close_short",
                              reason=f"SMA bullish cross or RSI {rsi:.0f} < {self.params['rsi_exit_os']}")
